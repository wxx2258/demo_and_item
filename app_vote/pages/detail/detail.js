...
Page({
  ...
  onLoad:function(options){
    ...
    that.setData({theme_id:options.id})
    var user_id = wx.getStorageSync('user_id');
    // console.log(options.id,user_id);
    selectData.queryitem(options.id,user_id).then(function(result_array){
      ...
      that.setData({itemArray:limitten});
      wx.hideToast();
    }).catch(function(err){
      console.error(err);
    })
